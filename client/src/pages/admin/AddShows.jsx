...
// import Title from '../../lib/admin/Title';
...
//           <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:-translate-y-1 transition duration-300`} onClick={()=>setSelectedMovie(movie.id)}>
...
const AddShows = () => {
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <div className='mt-6'>
        <label className='block text-sm font-medium mb-2'>Select Date and Time</label>
        <div className='flex flex-wrap gap-5 border border-gray-400 p-2 rounded-lg items-center'>
          <input
            type='datetime-local'
            value={dateTimeInput}
            onChange={(e) => setDateTimeInput(e.target.value)}
            className='outline-none rounded-md border px-2 py-1 text-sm'
          />
          <button
